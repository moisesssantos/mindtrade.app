...
export default function Operacoes() {
  ...
  // Query para buscar itens de todas as operações concluídas
  const { data: relatorios } = useQuery<any>({
    queryKey: ['/api/relatorios'],
  });

  // Filtrar apenas operações concluídas e ordenar por data da partida (mais recente primeiro)
  ...
  const getPartidaInfo = (partidaId: number) => {
    ...
    if (!partida) return { mandante: '', visitante: '', competicao: '', data: '', hora: '', dataFormatada: '' };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        <h1 className="text-3xl font-bold" data-testid="text-page-title">Operações Concluídas</h1>
      ...
      {operacoesConcluidas.length === 0 ? (
        ...
        <div className="grid gap-6">
          {operacoesConcluidas.map((operacao) => {
            ...
            return (
              <Card key={operacao.id} data-testid={`card-operacao-${operacao.id}`}>
                <CardHeader>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-sm text-muted-foreground flex flex-wrap items-center gap-x-4 gap-y-1">
                        ...
                        {operacao.dataConclusao && (
                          <div>
                            em {(() => {
                              const date = new Date(operacao.dataConclusao);
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                    ...
                    <div className="bg-muted/30 p-3 rounded-md">
                      ...
                      <div className={`text-lg font-mono font-bold flex items-center gap-1 ${
                        ...
                        {stats.resultadoTotal > 0 ? <TrendingUp className="w-4 h-4" /> : stats.resultadoTotal < 0 ? <TrendingDown className="w-4 h-4" /> : null}
