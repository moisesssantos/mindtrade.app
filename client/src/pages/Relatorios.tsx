...
export default function Relatorios() {
  ...
  // Queries
  const { data: relatoriosData, isLoading } = useQuery<{ operacoes: Operacao[]; itens: OperacaoItem[] }>({
  ...
  if (filtros.dataInicio || filtros.dataFim || (filtros.competicaoId && filtros.competicaoId !== 'all') || (filtros.equipeId && filtros.equipeId !== 'all')) {
    const operacoesFiltradas = operacoes.filter(op => {
      ...
      if (filtros.competicaoId && filtros.competicaoId !== 'all' && partida.competicaoId !== parseInt(filtros.competicaoId)) return false;
      if (filtros.equipeId && filtros.equipeId !== 'all') {
        const equipeIdNum = parseInt(filtros.equipeId);
        if (partida.mandanteId !== equipeIdNum && partida.visitanteId !== equipeIdNum) return false;
      }
    ...
    itensFiltrados = itens.filter(item => operacaoIdsFiltrados.has(item.operacaoId));
  }

  if (filtros.mercadoId && filtros.mercadoId !== 'all') {
    itensFiltrados = itensFiltrados.filter(item => item.mercadoId === parseInt(filtros.mercadoId));
  }

  if (filtros.estrategiaId && filtros.estrategiaId !== 'all') {
    itensFiltrados = itensFiltrados.filter(item => item.estrategiaId === parseInt(filtros.estrategiaId));
  }

  // Cálculos gerais
  const totalOperacoes = new Set(itensFiltrados.map(item => item.operacaoId)).size;
  const totalStake = itensFiltrados.reduce((acc, item) => acc + parseFloat(item.stake || '0'), 0);
  const lucroTotal = itensFiltrados.reduce((acc, item) => acc + parseFloat(item.resultadoFinanceiro || '0'), 0);
  const roi = totalStake > 0 ? (lucroTotal / totalStake) * 100 : 0;
  const operacoesLucrativas = itensFiltrados.filter(item => parseFloat(item.resultadoFinanceiro || '0') > 0).length;
  const taxaAcerto = itensFiltrados.length > 0 ? (operacoesLucrativas / itensFiltrados.length) * 100 : 0;
  const mediaPorOperacao = totalOperacoes > 0 ? lucroTotal / totalOperacoes : 0;
  const maiorGanho = itensFiltrados.length > 0 
    ? Math.max(...itensFiltrados.map(item => parseFloat(item.resultadoFinanceiro || '0')))
    : 0;

  // Agregações por mercado
  const porMercado = mercados.map(mercado => {
    const itensMercado = itensFiltrados.filter(item => item.mercadoId === mercado.id);
    const stakeTotal = itensMercado.reduce((acc, item) => acc + parseFloat(item.stake || '0'), 0);
    const lucro = itensMercado.reduce((acc, item) => acc + parseFloat(item.resultadoFinanceiro || '0'), 0);
  ...
  const porEstrategia = estrategias.map(estrategia => {
    const itensEstrategia = itensFiltrados.filter(item => item.estrategiaId === estrategia.id);
    const stakeTotal = itensEstrategia.reduce((acc, item) => acc + parseFloat(item.stake || '0'), 0);
    const lucro = itensEstrategia.reduce((acc, item) => acc + parseFloat(item.resultadoFinanceiro || '0'), 0);
  ...
  const seguiuPlanoSim = itensFiltrados.filter(item => item.seguiuPlano === true);
  const seguiuPlanoNao = itensFiltrados.filter(item => item.seguiuPlano === false);

  const stakeSeguiuSim = seguiuPlanoSim.reduce((acc, item) => acc + parseFloat(item.stake || '0'), 0);
  const lucroSeguiuSim = seguiuPlanoSim.reduce((acc, item) => acc + parseFloat(item.resultadoFinanceiro || '0'), 0);
  const roiSeguiuSim = stakeSeguiuSim > 0 ? (lucroSeguiuSim / stakeSeguiuSim) * 100 : 0;

  const stakeSeguiuNao = seguiuPlanoNao.reduce((acc, item) => acc + parseFloat(item.stake || '0'), 0);
  const lucroSeguiuNao = seguiuPlanoNao.reduce((acc, item) => acc + parseFloat(item.resultadoFinanceiro || '0'), 0);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="space-y-6">
        ...
        {/* Filtros */}
        <Card className={`p-6 ${isDarkMode ? "bg-transparent border hsl(var(--border))" : "bg-white border border-gray-200 shadow-sm"}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={filtros.competicaoId} onValueChange={(value) => setFiltros({ ...filtros, competicaoId: value })}>
            ...
            <div className="space-y-2">
              <Label>Mercado</Label>
              <Select value={filtros.mercadoId} onValueChange={(value) => setFiltros({ ...filtros, mercadoId: value })}>
            ...
            <div className="space-y-2">
              ...
              <Select value={filtros.estrategiaId} onValueChange={(value) => setFiltros({ ...filtros, estrategiaId: value })}>
        ...
        <Tabs defaultValue="geral" className="space-y-4">
            ...
            <Card className={`p-6 ${isDarkMode ? "bg-transparent border hsl(var(--border))" : "bg-white border border-gray-200 shadow-sm"}`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold font-mono">R$ {totalStake.toFixed(2).replace('.', ',')}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">Maior Ganho</p>
                  <p className={`text-2xl font-bold font-mono ${maiorGanho >= 0 ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <TabsContent value="mercado">
            <Card className="p-6">
              <h3 className="text-lg font-semibold mb-4">Performance por Mercado</h3>
              {porMercado.length === 0 ? (
                    ...
                    <TableBody>
                      {porMercado.map((row) => (
                        <TableRow key={row.mercado}>
                          ...
                          <TableCell className={`text-right font-mono ${row.lucro >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                            R$ {row.lucro.toFixed(2).replace('.', ',')}
                          </TableCell>
                          <TableCell className="text-right font-mono">{row.roi.toFixed(1).replace('.', ',')}%</TableCell>
                          <TableCell className="text-right font-mono">{row.operacoes}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {porEstrategia.map((row, index) => (
                        <TableRow key={index}>
                          <TableCell className="font-medium">{row.estrategia}</TableCell>
                          <TableCell className="text-muted-foreground">{row.mercado}</TableCell>
                          <TableCell className={`text-right font-mono ${row.lucro >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                            R$ {row.lucro.toFixed(2).replace('.', ',')}
                          </TableCell>
                          <TableCell className="text-right font-mono">{row.roi.toFixed(1).replace('.', ',')}%</TableCell>
          ...
          <TabsContent value="comportamental">
            <Card className="p-6">
              <h3 className="text-lg font-semibold mb-4">Análise Comportamental</h3>
              <div className="space-y-6">
                <div>
                  <h4 className="text-sm font-medium mb-3">Performance: Seguiu o Método</h4>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 rounded-lg border">
                      <p className="text-sm text-muted-foreground mb-1">Sim ({seguiuPlanoSim.length} itens)</p>
                      <p className={`text-xl font-bold font-mono ${lucroSeguiuSim >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {lucroSeguiuSim >= 0 ? '+' : ''}R$ {lucroSeguiuSim.toFixed(2).replace('.', ',')}
                      </p>
                      <p className="text-xs text-muted-foreground mt-1">ROI: {roiSeguiuSim.toFixed(1).replace('.', ',')}%</p>
                    </div>
                    <div className="p-4 rounded-lg border">
                      <p className="text-sm text-muted-foreground mb-1">Não ({seguiuPlanoNao.length} itens)</p>
                      <p className={`text-xl font-bold font-mono ${lucroSeguiuNao >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {lucroSeguiuNao >= 0 ? '+' : ''}R$ {lucroSeguiuNao.toFixed(2).replace('.', ',')}
                      </p>
                      <p className="text-xs text-muted-foreground mt-1">ROI: {roiSeguiuNao.toFixed(1).replace('.', ',')}%</p>
                    </div>
                  </div>
                </div>
                ...
                {porEstadoEmocional.length > 0 && (
                  <div>
                    <h4 className="text-sm font-medium mb-3">Performance por Estado Emocional</h4>
                    <div className="rounded-lg border">
                      <Table>
                        <TableHeader>
                          <TableRow>
